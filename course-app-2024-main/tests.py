...
response2 = requests.post(url + "/add_courses",
                          json={
                              "course_names": ["TDDD78", "TSEA28", "TDDD80", "TDDD79"]
                          })

register_user_1 = requests.post(url + "/register", json={'username': 'Yousef', 'password': 'hello12'})
login_user_1 =requests.post(url + "/user/login", json={'username': 'Yousef', 'password': 'hello12'})
token1 = login_user_1.json()['token']
header1 = {'Authorization': "Bearer " + token1}
courseid = "1"
comment1 = requests.post(url + f"/comment_course/{courseid}", headers=header1, json={"text" : " not very Nice course"})
like1 = requests.post(url  + "/like_course/TDDD78", headers=header1)
like2 = requests.post(url  + "/like_course/TSEA28", headers=header1)
'''
...
logout1 = requests.post(url  + "/user/logout", headers=header1)
...
register_user_2 = requests.post(url + "/register", json={'username': 'Zain', 'password': 'bye21'})
login_user_2 =requests.post(url + "/user/login", json={'username': 'Zain', 'password': 'bye21'})
login_user_2 =requests.post(url + "/user/login", json={'username': 'Zain', 'password': 'bye21'})

token2 = login_user_2.json()['token']
header2 = {'Authorization': "Bearer " + token2}

like2 = requests.post(url  + "/like_course/TDDD78", headers=header2)
like2 = requests.post(url + "/like_course/TSEA28", headers=header2)
comment2 = requests.post(url + "/comment_course/{course_id}", headers=header2, json={"text" : "Nice course"})
...
follow2 = requests.post(url + "/follow_user/Yousef", headers=header2)

show_followed_users_comments2 = requests.get(url + "/show_followed_users_comments", headers=header2)

response = requests.post("url/add_user/Yousef")
